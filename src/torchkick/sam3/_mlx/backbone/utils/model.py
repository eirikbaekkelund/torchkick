...
class TransformerWrapper(nn.Module):
    ...
    def _reset_parameters(self):
        def _init_fn(path, params):
            if params.ndim > 1:
                if "box_embed" not in path and "query_embed" not in path and "reference_points" not in path:
...
class LayerScale(nn.Module):
    def __init__(self, dim: int, init_values: Union[float, mx.array] = 1e-5, inplace: bool = False) -> None:
        super().__init__()
...
def gen_sineembed_for_position(pos_array, num_feats=256):
    ...
    if pos_array.shape[-1] == 2:
        ...
        pos_w = mx.stack((mx.sin(pos_w[:, :, 0::2]), mx.cos(pos_w[:, :, 1::2])), axis=3).flatten(2)

        h_embed = pos_array[:, :, 3] * scale
        pos_h = h_embed[:, :, None] / dim_t
        pos_h = mx.stack((mx.sin(pos_h[:, :, 0::2]), mx.cos(pos_h[:, :, 1::2])), axis=3).flatten(2)
