...
class PositionEmbeddingSine(nn.Module):
    ...
    def _encode_xy(self, x, y):
        ...
        pos_x = mx.stack((mx.sin(pos_x[:, 0::2]), mx.cos(pos_x[:, 1::2])), axis=2).flatten(1)
        pos_y = mx.stack((mx.sin(pos_y[:, 0::2]), mx.cos(pos_y[:, 1::2])), axis=2).flatten(1)
        return pos_x, pos_y
    ...
    def __call__(self, x: mx.array | tuple) -> mx.array:
        ...
        pos_x = x_embed[:, :, :, None] / dim_t
        pos_y = y_embed[:, :, :, None] / dim_t
        pos_x = mx.stack((mx.sin(pos_x[:, :, :, 0::2]), mx.cos(pos_x[:, :, :, 1::2])), axis=4).flatten(3)
        pos_y = mx.stack((mx.sin(pos_y[:, :, :, 0::2]), mx.cos(pos_y[:, :, :, 1::2])), axis=4).flatten(3)
